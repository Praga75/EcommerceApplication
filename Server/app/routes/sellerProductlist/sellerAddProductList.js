...
module.exports = (app, knex, acl) => {
    ...
    /**
     * @apiVersion 1.0.0
     * @api {post} /api/sellerAddProductList
     * @apiName sellerAddProductList
     * @apiGroup sellerProductlist
     *
     * @apiParam Sellerid
     *
    ...
    app.post('/api/sellerAddProductList', handler.asyncFn(async (req, res, next) => {
        let results = await sellerAddProductListQuery(req.body.Sellerid, null, null);
        res.send(results);
    }));

    const sellerAddProductListQuery = async (Sellerid, parentQuery, trx) => {
        ...
        query.where(function() {
            this.where('sellerProduct.sellerid', '=', Sellerid)
        })
    ...
    app.locals.services.sellerAddProductListQuery = sellerAddProductListQuery;
};
...
module.exports = (app, knex, acl) => {
    ...
    /**
     * @apiVersion 1.0.0
     * @api {post} /api/sellerProductlist
     * @apiName sellerProductlist
    ...
    app.post('/api/sellerProductlist', handler.asyncFn(async (req, res, next) => {
        let results = await sellerProductlistQuery(req.body.Sellerid, null, null);
        res.send(results);
    }));

    const sellerProductlistQuery = async (Sellerid, parentQuery, trx) => {
    ...
    app.locals.services.sellerProductlistQuery = sellerProductlistQuery;
};
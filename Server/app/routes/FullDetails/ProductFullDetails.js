...
module.exports = (app, knex, acl) => {
    ...
    /**
     * @apiVersion 1.0.0
     * @api {get} /api/ProductFullDetails
     * @apiName ProductFullDetails
     * @apiGroup FullDetails
     *
     *
    ...
    app.get('/api/ProductFullDetails', handler.asyncFn(async (req, res, next) => {
        let results = await ProductFullDetailsQuery(null, null);
        res.send(results);
    }));

    const ProductFullDetailsQuery = async (parentQuery, trx) => {
    ...
    app.locals.services.ProductFullDetailsQuery = ProductFullDetailsQuery;
};
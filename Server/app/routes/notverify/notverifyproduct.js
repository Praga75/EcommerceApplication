...
module.exports = (app, knex, acl) => {
    ...
    /**
     * @apiVersion 1.0.0
     * @api {get} /api/notverifyproduct
     * @apiName notverifyproduct
     * @apiGroup notverify
     *
     *
    ...
    app.get('/api/notverifyproduct', handler.asyncFn(async (req, res, next) => {
        let results = await notverifyproductQuery(null, null);
        res.send(results);
    }));

    const notverifyproductQuery = async (parentQuery, trx) => {
        ...
        query.where(function() {
            this.where('sellerProduct.isVerified', '=', 'Pending')
        })
    ...
    app.locals.services.notverifyproductQuery = notverifyproductQuery;
};